...
class modelOrders {
	async newOrder(token, info) {
		try {
			...
			const allow = await db.query(`select count(id) as counter from orders where status_id < 4 and users_id = $1`, [id])
			...
			const newOrder = await db.query(
				`insert into orders (description, projectType, users_id)
                values ($1, $2, $3) returning id`,
				[info.description, info.project, id]
			)
	...
	async updateStatusCanceled(info) {
		try {
			...
			await db.query(`update orders set status_id = 5 where id = $1`, [info.id])
