...
class modelProject {
  async getProject(id) {
    try {
      const info = await db.query(`select projects.id, projects.name, projects.description, projectPhoto.photoPath as photo from projects
        left join projectPhoto on projects.id = projectPhoto.projects_id
